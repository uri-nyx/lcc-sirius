...
int
cmpint2 (const void *x, const void *y)
{
  if (**(int **)x < **(int **)y)
    return -1;
  else if (**(int **)x > **(int **)y)
...
int
signext32 (int n, unsigned int val)
{
...
void
dumpString (unsigned int offset)
{
  long pos;
  int c;
...
void
addReloc (int segment, RelocRecord *relRec, Symbol **symMap)
{
  Reloc *rel;
...
Lelem *
newLelem (int cmp (const void *x, const void *y))
{
  Lelem *p;
...
void
foreachLelem (Lelem *list, void op (Lelem *x))
{
  Lelem *p = list;
  while (p != NULL)
    {
...
int
getmaxpos2 (int number)
{
  int i = 0;
  for (rbtuint_it (rbtitr, rvctree); !rbtuint_it_until_p (rbtitr, number);
       rbtuint_next (rbtitr))
    i++;
  return (i);
}
...
relocateSegments (void)
{
  ...
  if (!segStartDefined[SEGMENT_BSS])
    {
      segStart[SEGMENT_BSS] = segStart[SEGMENT_DATA] + segPtr[SEGMENT_DATA];
    }
...
Symbol *
newSymbol (char *name, char *filename, int debug, int debugtype,
           int debugvalue)
{
  Symbol *p;
...
Symbol *
lookupEnter (Symbol **table, char *name, char *filename, int debug,
...
void
walkSymbols (void (*fp) (Symbol *sym))
{
...
void
readString (unsigned int offset, char *buffer, int size)
{
  long pos;
  int c;
...
void
printliststring (Lelem *x)
{
  printf ("%s\r\n", (char *)x->valptr);
}

void
printdebSymbol (Symbol *s)
{
  int sep;
  char *ptr;
  unsigned int val;
  ...
  switch (s->debug)
    ...
    case DBG_FUNCBEG:
      ...
      (*funcptr)->startpos
          = roundup (sym->value + segStart[sym->type], FUNCALIGN);
      val = s->debugtype;
      strcpy (strtmp, tmp);
      strcat (strtmp, "_end");
...
writeData (void)
{
  ...
  rewind (dataFile);
  if (segStartDefined[SEGMENT_DATA])
    while (ftell (outFile) < segStart[SEGMENT_DATA] - segStart[SEGMENT_CODE])
      fputc (data, outFile);
  else
    for (i = 0; i < (2 * getmaxpos2 (segStart[SEGMENT_DATA]) & (SEGALIGN - 1));
         i++)
      fputc (data, outFile);
...
int
readNumber (char *str, unsigned int *np)
{
  int base;
  int value;
  int digit;
...
readlibsymbols (char *libname)
{
  ...
  pos = sizeof (arMagic) + sizeof (arhdr) + sizeof (int)
        + numSymbols * sizeof (Entry);
...
int
extractfilefromlib (char *libname, char *filename)
{
  unsigned int arMagic;
  FILE *in, *out;
  int i;
  ...
  while (libName)
    {
      ...
      printf ("Resolving:\r\n");
    restart:
      for (setcstr_it (itr, undefset); !setcstr_end_p (itr);
           setcstr_next (itr))
        {
          char *element = *setcstr_ref (itr);
