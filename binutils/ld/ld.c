...
void linkSymbol(Reloc *rel)
{
        ...
        if (debugLink) {
                ...
                printf("       (s:%s, o:%08X, m:%s, v:%08X --> %08X, b:%s)\n",
                       segName[rel->segment], rel->offset,
                       methodName[rel->method], rel->value,
...
void fixupRef(Reloc *rel, int scan)
{
        ...
        if (scan == 0) {
                ...
                if (rel->base.segment == SEGMENT_CODE)
                        ...
                        deltadst = 2 * getmaxpos2(segStart[SEGMENT_DATA]) &
                                   ~(SEGALIGN - 1);
                else
                        deltadst = 0;
        ...
        if (debugFixup) {
                printf("DEBUG: fixup (s:%s, o:%08X, m:%s, v:%08X), %08X --> %08X,delta "
                       "%i, dst %i, ofs %i\n",
                       segName[rel->segment], rel->offset,
                       methodName[rel->method], rel->value, value, final,
                       rvcdelta, deltadst, deltaofs);
        }
}
...
void writeHeader(void)
{
        ...
        if (!doElf) {
                ...
                int totsize = sizeof(EHdr) + sizeof(PHdr) +
                              segPtr[SEGMENT_CODE] + segPtr[SEGMENT_DATA] +
                              segPtr[SEGMENT_BSS];

                memcpy(EHdr.e_ident,
                       "\x7f\x45\x4c\x46\01\01\01\0\0\0\0\0\0\0\0\0", 16);
...
void writeData(void)
{
        ...
        if (segStartDefined[SEGMENT_DATA])
                ...
                for (i = 0; i < (2 * getmaxpos2(segStart[SEGMENT_DATA]) &
                                 (SEGALIGN - 1));
                     i++)
                        fputc(data, outFile);
...
int main(int argc, char *argv[])
{
        ...
        for (i = 1; i < argc; i++) {
                ...
                case 'r':
                        ...
                        if (!readNumber(argv[++i], ssp)) {
                                error("cannot read number given with option '-%s'",
                                      argp);
                        }
                        *ssdp = 1;
                        break;
                ...
                if (withDebug) {
                        ...
                        *strchr(tmp, '.') = '\0';
                        ...
                        if (JsondebFilein) {
                                ...
                                funcdict_splice(root->functions,
                                                rootas->functions);
                                typdefdict_splice(root->typdefs,
                                                  rootas->typdefs);
        ...
        while (libName) {
                ...
                for (setcstr_it(itr, defset); !setcstr_end_p(itr);
                     setcstr_next(itr)) {
