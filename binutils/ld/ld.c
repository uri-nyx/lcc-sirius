...
unsigned int toLE(unsigned int num)
{
        return ((num & 0xff000000) >> 24) | ((num & 0x00ff0000) >> 8) | ((num & 0x0000ff00) << 8) |
               (num << 24);
}

int setcstr_contains(setcstr_t set, const char *key) {
    setcstr_it_t it;
    for (setcstr_it(it, set); !setcstr_end_p(it); setcstr_next(it)) {
        if (strcmp(*setcstr_ref(it), key) == 0)
            return 1;
    }
    return 0;
}
...
void linkSymbol(Reloc *rel)
{
        ...
        if (debugLink) {
                printf("DEBUG: link '%s' (s:%s, v:%08X)\n", sym->name, segName[sym->type],
                       sym->value);
                printf("       (s:%s, o:%08X, m:%s, v:%08X --> %08X, b:%s)\n",
                       segName[rel->segment], rel->offset, methodName[rel->method], rel->value,
                       rel->value + sym->value, segName[sym->type]);
        }
...
{
        int i = 0;
        for (rbtuint_it(rbtitr, rvctree); !rbtuint_it_until_p(rbtitr, number); rbtuint_next(rbtitr))
                i++;
        return (i);
}
...
void fixupRef(Reloc *rel, int scan)
{
        ...
        if (scan == 0) {
                if (rel->segment == SEGMENT_CODE) {
                        ...
                        case METHOD_RL12:
                        case METHOD_RS12:
                                deltaofs = 2 * getmaxpos2(rel->offset - 4);
                                break;
                        default:
                                deltaofs = 2 * getmaxpos2(rel->offset);
                        }
                } else
                        deltaofs = 0;
                if (rel->base.segment == SEGMENT_CODE)
                        deltadst = 2 * getmaxpos2(rel->value);
                else if (!segStartDefined[SEGMENT_DATA])
        ...
        case METHOD_JALR12:
                ...
                break;
        case METHOD_RL12:
                value = rel->value - rel->offset - rvcdelta + 4 + segStart[rel->base.segment] -
                        segStart[rel->segment];
                fseek(file, rel->offset, SEEK_SET);
                fread(&final, sizeof(final), 1, file);

                value &= 0xFFF;
                final = toLE(final) | value;
                ...
                break;
        case METHOD_RH20:
                value = rel->value - rel->offset - rvcdelta + segStart[rel->base.segment] -
                        segStart[rel->segment];
                fseek(file, rel->offset, SEEK_SET);
                fread(&final, sizeof(final), 1, file);

                value = value >> 12;
                final = toLE(final) | (value & 0xFFFFF);
        ...
        /* output debugging info */
        if (debugFixup) {
                printf("DEBUG: fixup (s:%s, o:%08X, m:%s, v:%08X), %08X --> %08X,delta "
                       "%i, dst %i, ofs %i\n",
                       segName[rel->segment], rel->offset, methodName[rel->method], rel->value,
                       value, final, rvcdelta, deltadst, deltaofs);
        }
}
...
void relocateSegments(void)
{
        ...
        if (!segStartDefined[SEGMENT_DATA]) {
                unsigned long raw_data_start = segStart[SEGMENT_CODE] + segPtr[SEGMENT_CODE];
                if (doPageAlign) {
                        segStart[SEGMENT_DATA] = PAGE_ROUND(raw_data_start);
                } else {
                        segStart[SEGMENT_DATA] = raw_data_start;
                }
        }
        if (!segStartDefined[SEGMENT_BSS]) {
                unsigned long raw_bss_start = segStart[SEGMENT_DATA] + segPtr[SEGMENT_DATA];
                if (doPageAlign) {
                        segStart[SEGMENT_BSS] = PAGE_ROUND(raw_bss_start);
                } else {
                        segStart[SEGMENT_BSS] = raw_bss_start;
                }
        }
...
Symbol *newSymbol(char *name, char *filename, char *libname, int debug, int debugtype,
                  int debugvalue)
{
        Symbol *p;
...
Symbol *lookupEnter(Symbol **table, char *name, char *filename, char *libname, int debug,
                    int debugtype, int debugvalue)
{
        ...
        p = *table;
        if (p == NULL) {
                r      = newSymbol(name, filename, libname, debug, debugtype, debugvalue);
                *table = r;
                return r;
        }
        while (1) {
...
void readHeader(void)
{
        ...
        inHeader.magic   = toLE(inHeader.magic);
        inHeader.csize   = toLE(inHeader.csize);
        inHeader.dsize   = toLE(inHeader.dsize);
        inHeader.bsize   = toLE(inHeader.bsize);
        inHeader.crsize  = toLE(inHeader.crsize);
        inHeader.drsize  = toLE(inHeader.drsize);
        inHeader.symsize = toLE(inHeader.symsize);
        inHeader.strsize = toLE(inHeader.strsize);
...
void readSymbols(void)
{
        ...
        for (i = 0; i < n; i++) {
                ...
                sym = lookupEnter(&symbolTable, strBuf, NULL, NULL, symRec.debug, symRec.debugtype,
                                  symRec.debugvalue);
...
void writeHeader(void)
{
        ...
        if (!withHeader)
                return;
        if (!doElf) {
                outHeader.magic   = toLE(EXEC_MAGIC);
                outHeader.csize   = toLE(segPtr[SEGMENT_CODE]);
                outHeader.dsize   = toLE(segPtr[SEGMENT_DATA]);
                outHeader.bsize   = toLE(segPtr[SEGMENT_BSS]);
                outHeader.crsize  = toLE(0);
                outHeader.drsize  = toLE(0);
                outHeader.symsize = toLE(0);
                outHeader.strsize = toLE(0);
                fwrite(&outHeader, sizeof(ExecHeader), 1, outFile);
        } else {
                int totsize = sizeof(EHdr) + sizeof(PHdr) + segPtr[SEGMENT_CODE] +
                              segPtr[SEGMENT_DATA] + segPtr[SEGMENT_BSS];
...
void writeData(void)
{
        ...
        rewind(dataFile);
        if (segStartDefined[SEGMENT_DATA] || doPageAlign) {
                long target_pos = (long)segStart[SEGMENT_DATA] - (long)segStart[SEGMENT_CODE];
                if (target_pos > 0) {
                        while (ftell(outFile) < target_pos)
                                fputc(data, outFile);
                }
        } else
                for (i = 0; i < (2 * getmaxpos2(segStart[SEGMENT_DATA]) & (SEGALIGN - 1)); i++)
                        fputc(data, outFile);
...
int readlibsymbols(char *libname)
{
        ...
        if (fread(&arMagic, sizeof(arMagic), 1, libfile) != 1 || arMagic != AR_MAGIC) {
...
void usage(char *myself)
{
        ...
        fprintf(stderr, "         [-g]             write debug file in json\n");
        fprintf(stderr, "         [-o objfile]     set output file name\n");
        fprintf(stderr, "         [-m mapfile]     produce map file\n");
        fprintf(stderr, "         [-l library]     link library statically\n");
...
int main(int argc, char *argv[])
{
        ...
        if (liblist[0])
                listcstr_pop_back(&libName, liblist);
...
restart:
                for (setcstr_it(itr, undefset); !setcstr_end_p(itr); setcstr_next(itr)) {
                        ...
                        printf("Resolving symbol '%s' -> file '%s'\n", Sym->name, destfile);
                        printf("stat('%s') = %d\n", destfile, stat(destfile, &buf));

                        if (found && Sym) {
                                if (!setcstr_contains(loaded_members, Sym->filename)) {
                                        printf("Extracting %s for %s from archive %s\r\n",
                                               Sym->filename, Sym->name, Sym->library);
                                        setcstr_push(loaded_members, strdup(Sym->filename));
