...
unsigned int toBE(unsigned int num)
{
...
int getNextToken(void)
{
        ...
        if (isalpha((int)*lineptr) || *lineptr == '_' || *lineptr == '.') {
                p = tokenvalString;
                while (isalnum((int)*lineptr) || *lineptr == '_' ||
                       *lineptr == '.') {
                        *p++ = *lineptr++;
                }
                *p = '\0';
        ...
        if (isdigit((int)*lineptr)) {
                ...
                while (isxdigit((int)*lineptr)) {
                        ...
                        if (digit >= base) {
                                error("illegal digit value %d in line %d",
                                      digit, lineno);
                        }
                        tokenvalNumber *= base;
                        tokenvalNumber += digit;
        ...
        if (*lineptr == '\'') {
                ...
                if (!isprint((int)*lineptr)) {
                        error("cannot quote character 0x%02X in line %d",
                              *lineptr, lineno);
                }
        ...
        if (*lineptr == '\"') {
                ...
                while (1) {
                        if (*lineptr == '\n' || *lineptr == '\0') {
                                error("unterminated string constant in line %d",
                                      lineno);
                        }
                        if (!isprint((int)*lineptr)) {
...
static char *tok2str[] = { "end-of-line", "label",    "identifier", "string",
                           "number",      "register", "+",          "-",
                           "*",           "/",        "%",          "<<",
                           ">>",          "(",        ")",          ",",
                           "~",           "&",        "|",          "^" };
...
void addFixup(Symbol *s, int segment, unsigned int offset, int method,
              int value)
{
        Fixup *f;
...
static void linkSymbol(Symbol *s)
{
        ...
        if (s->status == STATUS_GLOBREF) {
                if (s->fixups != NULL) {
                        error("local fixups detected with global symbol '%s'",
                              s->name);
                }
        } else {
                if (debugFixup) {
...
Value parseUnaryExpression(void)
{
        ...
        if (token == TOK_PLUS) {
                ...
                v = parseUnaryExpression();
                if (v.sym != NULL) {
                        error("cannot negate symbol '%s' in line %d",
                              v.sym->name, lineno);
                }
                v.con = -v.con;
                ...
                v = parseUnaryExpression();
                if (v.sym != NULL) {
                        error("cannot complement symbol '%s' in line %d",
                              v.sym->name, lineno);
                }
                v.con = ~v.con;
        } else {
...
Value parseMultiplicativeExpression(void)
{
        ...
        while (token == TOK_STAR || token == TOK_SLASH ||
               token == TOK_PERCENT) {
                if (token == TOK_STAR) {
                        getToken();
                        ...
                        v2 = parseUnaryExpression();
                        if (v1.sym != NULL || v2.sym != NULL) {
                                error("division of symbols not supported, line %d",
                                      lineno);
                        }
                        if (v2.con == 0) {
...
Value parseAdditiveExpression(void)
{
        ...
        while (token == TOK_PLUS || token == TOK_MINUS) {
                if (token == TOK_PLUS) {
                        ...
                        if (v1.sym != NULL && v2.sym != NULL) {
                                error("addition of symbols not supported, line %d",
                                      lineno);
                        }
                        if (v2.sym != NULL) {
                                v1.sym = v2.sym;
...
Value parseShiftExpression(void)
{
        ...
        while (token == TOK_LSHIFT || token == TOK_RSHIFT) {
                if (token == TOK_LSHIFT) {
                        ...
                        if (v1.sym != NULL || v2.sym != NULL) {
                                error("shifting of symbols not supported, line %d",
                                      lineno);
                        }
                        v1.con <<= v2.con;
...
Value parseAndExpression(void)
{
        ...
        while (token == TOK_AMPER) {
                ...
                if (v2.sym != NULL) {
                        error("bitwise 'and' of symbols not supported, line %d",
                              lineno);
                }
                v1.con &= v2.con;
        }
...
Value parseExclusiveOrExpression(void)
{
        ...
        while (token == TOK_CARET) {
                ...
                if (v2.sym != NULL) {
                        error("bitwise 'xor' of symbols not supported, line %d",
                              lineno);
                }
                v1.con ^= v2.con;
        }
...
Value parseInclusiveOrExpression(void)
{
        ...
        while (token == TOK_BAR) {
                ...
                if (v2.sym != NULL) {
                        error("bitwise 'or' of symbols not supported, line %d",
                              lineno);
                }
                v1.con |= v2.con;
        }
        ...
        case DW_TAG_variable:
                ...
                if (tokenvalNumber == 1) {
                        ...
                        if (tokenvalNumber == 1) {
                                ...
                                getToken();
                                funcvar->pos = tokenvalNumber;
                                funcvararr_push_back(func->stacklocals,
                                                     funcvar);
                        }
                }
        }
...
{
        // expect(TOK_STRING);
        if (strncmp(tokenvalString, ".debug_abbrev", strlen(".debug_abbrev")) ==
            0)
                asm_dbgabrev();
        else if (strncmp(tokenvalString, ".debug_info",
                         strlen(".debug_info")) == 0)
                asm_dbginfo();
        else if (strncmp(tokenvalString, ".debug_pubnames",
                         strlen(".debug_pubnames")) == 0)
                asm_dbgpubnames();
...
void dotLoc(unsigned int code)
{
        ...
        if (token != TOK_EOL) {
                w = parseExpression();
                if (w.sym != NULL) {
                        error("absolute expression expected in line %d",
                              lineno);
                }
        }
        if (debug) {
...
void dotSet(unsigned int code)
{
        ...
        if (globalnew->status == STATUS_DEFINED) {
                error("'%s' already not defined, line %d", globalnew->name,
                      lineno);
        }
        getToken();
        expect(TOK_COMMA);
...
void dotAlign(unsigned int code)
{
        ...
        if (v.con != 0 && (v.con & (v.con - 1)) == 0) {
                padding_to_emit =
                        ((segPtr[currSeg] + (v.con - 1)) & ~(v.con - 1)) -
                        segPtr[currSeg];
                ...
                error("epxression in alignment must be a power of 2, line %d",
                      lineno);
        }
...
void dotWord(unsigned int code)
{
        ...
        while (1) {
                ...
                if (v.sym == NULL) {
                        ...
                        addFixup(v.sym, currSeg, segPtr[currSeg], METHOD_W32,
                                 v.con);
                        emitWord(0);
                }
                if (token != TOK_COMMA) {
...
void dotSet2(unsigned int code)
{
        ...
        if (symbol->status != STATUS_UNKNOWN) {
                error("symbol '%s' multiply defined in line %d", symbol->name,
                      lineno);
        }
        getToken();
        expect(TOK_COMMA);
...
void format1JAL(unsigned int code)
{
        ...
        if (token == TOK_IREGISTER) {
                ...
                dst = 1;
        v = parseExpression();
        // IT SEGFAULTS HERE IF GIVEN A NUMBER BECAUSE v.sym == NULL, and addFixup dereferences it :(
...
void asmModule(void)
{
        ...
        while (fgets(line, LINE_SIZE, inFile) != NULL) {
                ...
                while (token == TOK_LABEL) {
                        if (debug &&
                            (strcmp(currfuncnameend, tokenvalString) == 0))
                                funcdict_set_at(root->functions, funckey, func);
                        label = deref(
                                lookupEnter(tokenvalString, LOCAL_TABLE, 0));
...
void writeObjectBE(const void *object, size_t size, FILE *stream)
{
        int   i;
...
int main(int argc, char *argv[])
{
        ...
        do {
                ...
                linkLocals();
                if (debug)
                        typdefdict_set_at(root->typdefs, func->filename,
                                          typdefarr);
        } while (++i < argc);
        writeDummyHeader();
        writeCode();
