...
/*
 * sirius.md -- Sirius (fantasy) CPU back-end specification
 *
 * register usage:
 *   x0   always zero
...
stmt:	ASGNI8(addr,pair)	"\tsw x%1, %0\n\tsw x%1^, 4%0\n"	1
...
stmt:	ASGNU8(addr,pair)	"\tsw x%1, %0\n\tsw x%1^, 4%0\n"	1

stmt: ASGNI4(addr, CVII4(INDIRI8(VREGP)))  "\tsw x%1, %0\n" 0
...
pair:  INDIRI8(addr)   "\tlw x%c,%0\n\tlw x%c^, 4%0\n" 1
pair:  INDIRU8(addr)   "\tlw x%c,%0\n\tlw x%c^, 4%0\n" 1
...
reg: NEI4(reg,reg)  "\txor x%c,x%0,x%1\n\tsltu x%c,x0,x%c\n"   2
reg: NEU4(reg,reg)  "\txor x%c,x%0,x%1\n\tsltu x%c,x0,x%c\n"   2

reg: LTI4(reg,reg)  "\tslt x%c,x%0,x%1\n"    1
reg: LTU4(reg,reg)  "\tsltu x%c,x%0,x%1\n"   1

reg: GEI4(reg,reg)  "\tslt x%c,x%0,x%1\n\txori x%c,x%c,1\n"   2
reg: GEU4(reg,reg)  "\tsltu x%c,x%0,x%1\n\txori x%c,x%c,1\n"  2

reg: GTI4(reg,reg)  "\tslt x%c,x%1,x%0\n"    1
reg: GTU4(reg,reg)  "\tsltu x%c,x%1,x%0\n"   1
...
stmt: CALLB(reg, reg)  "\tjalr x1, 0(x%0)\n"  1
...
static Symbol argreg(int argno, int offset, int ty, int sz, int ty0) {
  assert((offset & 3) == 0);
  if (offset + sz > 24 || sz > 8) {
    return NULL;
  }

  if (sz == 8) {
    return ireg2[(offset / 4) + 12];
  }
...
static void progbeg(int argc, char *argv[]) {
  ...
  for (i = 0; i < 32; i += 2) {
    ireg2[i] = mkreg("%d", i, 3, IREG); // 3 = Weight/Size of 2 slots
}
...
static void blkfetch(int size, int off, int reg, int tmp) {
  ...
  if (size == 8) {
    /* tmp must be an even register for the pair */
    print("\tlw x%d,%d(x%d)\n", tmp, off, reg);
    print("\tlw x%d^,%d(x%d)\n", tmp, off + 4, reg);
  }
}
...
static void blkloop(int dreg, int doff,
                    int sreg, int soff,
                    int size, int tmps[]) {
    // Sirius 'copy' instruction uses registers as pointers.
    // If lcc provides an offset, we must apply it first.
    if (doff != 0) print("\taddi x%d,x%d,%d\n", dreg, dreg, doff);
    if (soff != 0) print("\taddi x%d,x%d,%d\n", sreg, sreg, soff);

    // Load byte count into the borrowed temp register (x7)
    print("\tli x%d,%d\n", tmps[0], size);
    
    // copy [src], [dst], [count_bytes]
...
static void emit2(Node p) {
  ...
  switch (specific(p->op)) {
    ...
    case ARG+U:
      ...
      sz = opsize(p->op);
      if (sz == 8) {
        q = argreg(p->x.argno, p->syms[2]->u.c.v.i, I, 8, ty0);
...
static void target(Node p) {
  ...
  switch (specific(p->op)) {
    ...
    case CVU + I:    case CVU + U:
      if (opsize(p->op) == 8) {
          setreg(p, ireg2w);
      } else {
          setreg(p, iregw);  // 32-bit result
          // The child's register class is already determined by its own type
      }
      break;
    case EQ + I:     case EQ + U:
    case NE + I:     case NE + U:
    ...
    case CNST+U:
    if (opsize(p->op) == 8) {
        /* Force 64-bit constants into a register pair */
        setreg(p, ireg2w); 
    } else if (range(p, 0, 0) == 0) {
        setreg(p, ireg[0]);
        p->x.registered = 1;
      }
      break;
